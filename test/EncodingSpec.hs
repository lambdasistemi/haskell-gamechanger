...
{- | Harness over @test\/fixtures\/resolver-urls\/*.txt@. Each fixture
pairs a pinned LZMA1 resolver URL with its sibling canonical JSON:

- decode the @.txt@ via 'decodeResolverUrl';
- re-encode the resulting 'Script' to JSON via aeson;
- compare structurally against the committed @.json@.

Fixtures are generated by @cabal run gen-fixtures@ from the #6
goldens. They pin the current byte layout of the encoder so we
catch drift (liblzma version bump, header change, base64 variant)
even when the round-trip property would still pass.
-}
resolverUrlFixtures :: IO TestTree
resolverUrlFixtures = do
    paths <- findByExtension [".txt"] "test/fixtures/resolver-urls"
    pure $ testGroup "fixture decode matches sibling JSON" (map oneFixture paths)

oneFixture :: FilePath -> TestTree
oneFixture txtPath = testCase (takeBaseName txtPath) $ do
    urlText <- T.strip <$> TIO.readFile txtPath
    url <- case mkResolverUrl urlText of
        Left e -> assertFailure $ "mkResolverUrl: " <> show e
        Right u -> pure u
    decoded <- case decodeResolverUrl url of
        Left e -> assertFailure $ "decodeResolverUrl: " <> show e
        Right s -> pure s
    expectedBytes <- BSL.readFile (replaceExtension txtPath ".json")
    expected <- case Aeson.eitherDecode expectedBytes :: Either String Script of
        Left e -> assertFailure $ "sibling JSON is not a Script: " <> e
        Right s -> pure s
    -- Compare via structural Script equality (the library's Eq), not
    -- raw bytes: aeson's key ordering is not stable enough for byte
    -- compare to be useful here.
    decoded @?= expected
